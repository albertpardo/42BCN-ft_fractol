...
/*   Updated: 2023/09/08 11:34:30 by apardo-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/*
 * Based on 4_mandel_V0.c
 *
...
//			if (inoutmandel(c, maxiter) > 2)
//				printf("(%d,%d)-> %d/%d > %X \n", x, y, inoutmandel(c, maxiter), maxiter, setcolor(inoutmandel(c, maxiter), maxiter));
			my_put_pixel(img, x, y, setcolor(img->active_palete, inoutmandel(c, maxiter), maxiter));
			x++;
		}
		y++;
	}
